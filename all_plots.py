...
plt.annotate('Initial: 42.86%', xy=(1, 42.86), xytext=(3, 38), fontsize=11, ha='center', color='red')
plt.annotate('Final: 75.00%\n(+32.14pp)', xy=(25, 75), xytext=(22, 80), fontsize=11, ha='center', color='green', weight='bold')
...
plt.close()
print("✅ 1. Progressive Learning Plot Created")

# 2. Cross-Domain Performance Plot
...
bars1 = ax.bar(x - width/2, accuracy, width, label='Model Accuracy (%)', color=['#FF6B6B', '#4ECDC4', '#45B7D1'], alpha=0.8)
bars2 = ax.bar(x + width/2, detection, width, label='Best Attack Detection (%)', color=['#FF9999', '#7FDDDD', '#7FC7E8'], alpha=0.9)
...
plt.close()
print("✅ 2. Cross-Domain Performance Plot Created")

# 3. Literature Comparison Plot
...
bars1 = ax.bar(x - width/2, literature, width, label='Literature Best', color='#FFB6C1', alpha=0.8)
bars2 = ax.bar(x + width/2, our_method, width, label='Our Method', color='#32CD32', alpha=0.8)
...
for i, (lit, our, imp) in enumerate(zip(literature, our_method, improvements)):
    ...
    ax.text(i, (lit + our) / 2, f'+{imp:.1f}pp', ha='center', va='center', fontsize=11, weight='bold', color='red')
...
plt.close()
print("✅ 3. Literature Comparison Plot Created")

# 4. Non-IID Resilience Comparison
...
bars1 = ax.bar(x - width, iid_acc, width, label='IID Baseline', color='#2E8B57', alpha=0.8)
bars2 = ax.bar(x, dirichlet_acc, width, label='Dirichlet Non-IID', color='#FF6347', alpha=0.8)
bars3 = ax.bar(x + width, label_skew_acc, width, label='Label Skew Non-IID', color='#4169E1', alpha=0.8)
...
plt.close()
print("✅ 4. Non-IID Resilience Plot Created")

# 5. Comprehensive Overview (4 subplots)
...
plt.suptitle('Federated Learning Security: Comprehensive Research Overview', fontsize=18, weight='bold', y=0.98)
plt.tight_layout()
plt.savefig('plots/comprehensive_overview.png', dpi=300, bbox_inches='tight')
plt.close()
print("✅ 5. Comprehensive Overview Plot Created")

# 6. Attack Detection Performance by Domain
attacks = ['Label\nFlipping', 'Noise\nAttack', 'Sign\nFlipping', 'Partial\nScaling', 'Scaling\nAttack']
alzheimer_perf = [75.00, 60.00, 57.14, 50.00, 42.86]
mnist_perf = [27.59, 30.00, 47.37, 69.23, 30.00]
cifar_perf = [40.00, 100.00, 45.00, 100.00, 100.00]
...
bars1 = ax.bar(x - width, alzheimer_perf, width, label='ALZHEIMER', color='#FF6B6B', alpha=0.8)
bars2 = ax.bar(x, mnist_perf, width, label='MNIST', color='#4ECDC4', alpha=0.8)
bars3 = ax.bar(x + width, cifar_perf, width, label='CIFAR-10', color='#45B7D1', alpha=0.8)

ax.set_title('Attack Detection Performance by Domain', fontsize=16, weight='bold')
ax.set_ylabel('Detection Precision (%)', fontsize=14)
...
plt.tight_layout()
plt.savefig('plots/attack_detection_by_domain.png', dpi=300, bbox_inches='tight')
plt.close()
print("✅ 6. Attack Detection by Domain Plot Created")
...
print("3. literature_comparison.png")
print("4. noniid_resilience_comparison.png")
print("5. comprehensive_overview.png")
print("6. attack_detection_by_domain.png")
print("\n🎯 All plots are ready for paper submission!") 